...
/*
 A modified version of ttySetCbreak for use with MCUs. Places it in Cbreak mode,
 but doesn't allow one character at a time via the c_cc structure, triggering
 interrupt, but instead uses a 'break' signal, set by brkint. Allows us to
 do serial i/o in a non canonical (not line based mode) */

int
ttySetCbreakMCU(int fd, struct termios *prevTermios)
{
    struct termios t;
